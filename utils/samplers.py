...
from .plot_utils import mesh_plotter_2d_notrii
...
@sampler_decorator
class SamplerWithMy(InterpolationSampler):
    """
    Implement of PINN-DMIS
    """
    ...
    def __init__(
        ...
        super(SamplerWithMy, self).__init__(data, reweighting)
    ...
    def compute_scores(self):
        ...
        while True:
            ...
            scores_tensor = torch.cat([scores_tensor, _loss], dim=0)
            _index = last_index   
        if self.fudata is not None:
            self.fudata.requires_grad_()
            fu_loss= self.loss_func(self.model, self.fudata.cuda())
            scores_tensor=torch.cat([scores_tensor,fu_loss],dim=0)
    ...
    def mesh_update(self):
        ...
        if self.step_count!=0:
            self.model.eval()
            self.fudata,self.fudata_np=fusample(self.model)
            self.model.train()
        seed_points = self.data_np[self.seed_indxs]
        if self.fudata_np is not None:
            seed_points = np.append(seed_points,self.fudata_np,axis=0)
        n_dim = self.data_np.shape[1]
...
@sampler_decorator
class SamplerWithMyXiaobo(InterpolationSampler):
    """
    Implement of PINN-DMIS
    """
    ...
    def __init__(
        ...
        super(SamplerWithMyXiaobo, self).__init__(data, reweighting)
    ...
    def mesh_update_check(self):
        ...
        if ((cos_sim < self.mesh_update_thres) or self.step_count%5000==0):
            #or ((self.step_count>1000 and self.step_count%5000==0))
            self.mesh_update_flag = True
            self.log.info("change mesh")
    ...
    def mesh_update(self):
        ...
        if self.step_count!=0:
            self.model.eval()
            self.fudata,self.fudata_np=fusample(self.model,step=self.step_count)
            self.model.train()
        seed_points = self.data_np[self.seed_indxs]
        if self.fudata_np is not None:
            seed_points = np.append(seed_points,self.fudata_np,axis=0)
...
@sampler_decorator
class NabianMethod_xiaobo(InterpolationSampler):
    def __init__(
            self,
            data,
            reweighting,
        ...
        super(NabianMethod_xiaobo, self).__init__(data, reweighting)
    ...
    def compute_scores(self):
        ...
        # interpolation
        # ---------------
        interp_scores = interpolate.griddata(self.seed_points,
                                             self.seed_scores_t,
                                             self.data_np[:self.data_n],
                                             method="nearest")
        interp_scores -= np.min(interp_scores)
    ...
    def mesh_update(self):
            ...
            self.fudata,self.fudata_np=fusample(self.model,step=self.step_count)
            self.model.train()
            self.seed_points = self.data_np[self.seed_indxs]
            if self.fudata_np is not None:
                self.seed_points = np.append(self.seed_points,self.fudata_np,axis=0)
            n_dim = self.data_np.shape[1]
            save_dict = {
            ...
            output_full_path = os.path.join("./mesh_data", "{}_{}.npz".format("mesh", self.step_count))
            np.savez(output_full_path, **save_dict)
            mesh_plotter_2d_notrii(self.seed_points,self.step_count)
